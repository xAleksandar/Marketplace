      ...
      <div>
        {loading ? (
          ...
          <div className="Container">
            {myitems.map(x => <li key={x.name}><span><HomeItem image={x.image} forSell={x.forSell} tokenid={x.tokenid} price={x.price} bidPrice={x.bidPrice} collection={x.collection} marketplace={marketplace} name={x.name} signer={signer} /></span></li>)}
          </div>
        )}
      </div>
    </div>
